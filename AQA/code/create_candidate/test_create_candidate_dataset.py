...
from transformers import get_cosine_schedule_with_warmup, DataCollatorWithPadding, get_linear_schedule_with_warmup
import cupy as cp
...
# PATH
INPUT_DIR = 'data/raw/'
OUTPUT_MODEL_DIR = 'model/stage1/test_emb_model/'
...
def load_data(data):
  if data == 'train':
    ...
    with open(INPUT_DIR + f"qa_{data}_wo_ans_new.txt", 'r', encoding='utf-8') as file:
        df = pd.DataFrame()
        for idx, line in enumerate(file):
          data = json.loads(line)
          df.loc[idx,'question'] = data["question"]
...
# Get neighbors
# =========================================================================================
def get_neighbors(paper, query, cfg):
    ...
    # pretrained model
    state = torch.load(OUTPUT_MODEL_DIR+f"{Config.model_name.replace('/', '-')}_epoch{Config.inf_epoch}.pth",
    ...
    for k in range(len(indices)):
        ...
        predictions.extend(p[:Config.top_n])
        preds_text.extend(t[:Config.top_n])
...
def main():
    ...
    test_llm_answer= pd.read_csv(LLM_ANSWER_DIR + "test_llm_answer_question_body.csv")
    # llm summarize question
    train_llm_summarize = pd.read_csv(LLM_SUMMARIZE_DIR + "train_llm_summarize_question_body.csv")
    test_llm_summarize = pd.read_csv(LLM_SUMMARIZE_DIR + "test_llm_summarize_question_body.csv")
    # test
    test = load_data('test')
    test['q_id'] = np.arange(len(test))
    ...
    test_llm_answer = test_llm_answer.dropna(subset=['question','body','create_llm_answer_question']).reset_index(drop=True)
    test_llm_answer = test_llm_answer.drop_duplicates(subset=['question']).reset_index(drop=True)
    ...
    test_llm_summarize = test_llm_summarize.dropna(subset=['question','body','create_llm_summarize']).reset_index(drop=True)
    test_llm_summarize = test_llm_summarize.drop_duplicates(subset=['question']).reset_index(drop=True)
    ...
    # test
    test = test.merge(test_llm_answer[['question', 'create_llm_answer_question']], how='left', on='question')
    test = test.merge(test_llm_summarize[['question', 'create_llm_summarize']], how='left', on='question')
    test['create_llm_answer_question'] = test['create_llm_answer_question'].fillna('')
    test['create_llm_summarize'] = test['create_llm_summarize'].fillna('')
    # add prefix
    test['question'] = '<question>'  + ' ' + test['question']
    test['create_llm_answer_question'] = '<llm answer>'  + ' ' + test['create_llm_answer_question']
    test['create_llm_summarize'] = '<llm summarize>'  + ' ' + test['create_llm_summarize']
    test['question_llm_answer_summarize'] = test['question'] + ' ' + test['create_llm_answer_question'] + ' ' + test['create_llm_summarize']
    ...
    ## create passage
    # old paper
    old_papers = pd.read_json(INPUT_DIR + "pid_to_title_abs_new.json")
    old_papers = old_papers.T.reset_index().rename(columns={'index':'pids'})
    # new paper
    new_papers = pd.read_json(INPUT_DIR + "pid_to_title_abs_update_filter.json")
    new_papers = new_papers.T.reset_index().rename(columns={'index':'pids'})

    for col in new_papers.select_dtypes(include=['object']).columns:
        new_papers[col] = new_papers[col].apply(remove_special_chars)

    print('Before')
    print(' old_papers len',len(old_papers['pids'].unique()))
    print(' new_papers len',len(new_papers['pids'].unique()))
    old_papers_unique_pids = old_papers['pids'].unique()
    new_papers_unique_pids = new_papers['pids'].unique()
    print('After')
    print(' new_papers len', len(new_papers[~new_papers['pids'].isin(old_papers_unique_pids)]))

    new_papers = new_papers[~new_papers['pids'].isin(old_papers_unique_pids)].reset_index(drop=True)
    new_papers['title'] = '<title>'  + ' ' + new_papers['title']
    new_papers['abstract'] = '<abstract>'  + ' ' + new_papers['abstract']
    new_papers['title_abstract'] = new_papers['title'] + ' ' + new_papers['abstract']
    
    # title + abstract
    tmp_title_abstract = new_papers[['title_abstract', 'pids']]
    tmp_title_abstract = tmp_title_abstract.rename(columns={'title_abstract': 'text'})
    tmp_title_abstract = tmp_title_abstract[['text', 'pids']]

    # passage
    passage = tmp_title_abstract.copy()
    ...
    for epoch in tqdm(range(Config.epochs)):
        ...
        for step, (question_body, title_abs) in enumerate(train_loader):
            ...
            batch_size = Config.batch_size #labels.size(0)
        ...
        torch.save({'model':model.state_dict()},OUTPUT_MODEL_DIR+f"{Config.model_name.replace('/','-')}_epoch{epoch+1}.pth")
    ...
    test = get_neighbors(passage, test, Config)
    test['pids_prediction'] = test['pids_prediction'].apply(lambda x: list(x.split(',')))
    test['preds_text'] = test['preds_text'].apply(lambda x: list(x.split('<pred_separator>')))

    # postprocess
    test_pp = test.explode(['pids_prediction','preds_text']).reset_index(drop=True)
    test_pp = test_pp.drop_duplicates(subset=['q_id', 'pids_prediction']).reset_index(drop=True)
    test_pids_prediction = test_pp.groupby('q_id')['pids_prediction'].apply(lambda x: [pred for pred in x][:Config.pp_top_n]).reset_index()
    test_preds_text = test_pp.groupby('q_id')['preds_text'].apply(lambda x: [pred for pred in x][:Config.pp_top_n]).reset_index()
    test = test.drop(['pids_prediction', 'preds_text'],axis=1)
    test_pp = test_pids_prediction.merge(test_preds_text, how='left', on='q_id')
    test_pp = test_pp.merge(test, how='left', on='q_id')

    # ------------------------------ #

    # Create test dataset
    test_pp = build_training_set(new_papers, test_pp, Config)

    # Save train set to disk to train on another notebook
    test_pp.to_csv(OUTPUT_DIR + 'test_candidates.csv', index = False)
