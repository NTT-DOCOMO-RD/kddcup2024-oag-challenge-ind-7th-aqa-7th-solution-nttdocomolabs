...
def generate_prompt(question):
  ...
  prompt = """
  ### Instruction
  You are an expert in summarizing technical questions posted on the Internet in an easy-to-understand manner.
...
def main():
    # load
    qa_test = load_data('test')

    # remove_links
    qa_test['body'] = qa_test['body'].apply(remove_links)

    # remove_newlines
    qa_test['body'] = remove_newlines(qa_test['body'])

    # get head tail 
    text_length = Config.max_length
    qa_test['question_body'] = qa_test['question'] + '[SEP]' + qa_test['body']
    qa_test['question_body'] = qa_test['question_body'].apply(lambda x:shorten_text(x, text_length))
    ...
    # generate
    create_llm_summarize_question = qa_test.copy()
    for idx in range(len(create_llm_summarize_question)):

        question = create_llm_summarize_question['question_body'][idx]
        ...
        create_llm_summarize_question.loc[idx,'create_llm_summarize'] = generated_text[len(prompt):]
    ...
    # save
    create_llm_summarize_question.to_csv(OUTPUT_DIR + 'test_llm_summarize_question_body.csv', index=False)
