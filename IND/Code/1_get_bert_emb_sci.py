...
def sci(row, proc, df_master, model, tokenizer):
    author_id = row["id"]
    if proc == "train":
...
def main():
    ...
    for index, row in df_train_master.iterrows():
        print(index)
        dfs_train.append(sci(row, "train", df_master, model, tokenizer))
    df_train = pd.concat(dfs_train)
    print(df_train.shape)
    print(df_train.head())
    df_train.to_parquet("../test_feature/train_scibert_nli.parquet", index=False)

    dfs_test = []
    for index, row in df_test_master.iterrows():
        print(index)
        dfs_test.append(sci(row, "test", df_master, model, tokenizer))
    df_test = pd.concat(dfs_test)
    print(df_test.shape)
    print(df_test.head())

    df_test.to_parquet("../test_feature/test_scibert_nli.parquet", index=False)
